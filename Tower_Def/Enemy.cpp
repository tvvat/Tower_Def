...
namespace cry {
	...
	std::pair<int,int> Enemylight::moving(const Playng_field& field, std::pair<int,int> step,int xy) {
		...
		auto dist0 = std::hypot(field.palacecoordin.first - step.first, field.palacecoordin.second - (step.second - 1));
		auto dist = std::hypot(field.palacecoordin.first - step.first, field.palacecoordin.second - step.second);
		auto dist1 = std::hypot(field.palacecoordin.first - step.first, field.palacecoordin.second - (step.second + 1));
		std::map<int, std::pair<int, int>> tmp1;
		tmp1[dist0] = std::make_pair(step.first, step.second - 1);
		tmp1[dist] = std::make_pair(step.first, step.second);
		tmp1[dist1] = std::make_pair(step.first, step.second + 1);
		std::cout << dist0 << ' ' << dist << ' ' << dist1 << '\n';
		auto res = tmp1.begin();
		tmp = field.quad[res->second.second][res->second.first];
		if ((tmp.getType() == plain))
			...
			if ((tmp.getType() == plain) )
				...
				if ((tmp.getType() == plain) )
					...
					tmp = field.quad[step.second + 1][step.first - 1];
					if ((tmp.getType() == plain) )
						return stepup;
					else
