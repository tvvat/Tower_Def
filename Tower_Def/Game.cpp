...
namespace cry {
	...
	Playng_field::Playng_field() :lenght(50), width(50) {
		...
		quad.assign(width, std::vector<landscape>(lenght));
	...
	int Playng_field::calculateTrajectory() {
		std::map<std::pair<int, int>, std::shared_ptr<Enemy> > tmp;
		std::map<std::pair<int, int>, typeenemy > tmptype;
		auto type = typewhere.begin();


		for (auto it = enemywhere.begin(); it != enemywhere.end(); ++it, ++type) {
			if (it->first.first < palacecoordin.first) {
				auto newstep = it->second->moving(*this, std::make_pair(it->first.first + 1, it->first.second), 0);
				if (newstep == palacecoordin) {
					enemypalace.push_back(it->second);
				}
				else {
				...
				if (it->first.first == palacecoordin.first) {
					auto newstep = it->second->moving(*this, std::make_pair(it->first.first, it->first.second), 1);
					if (newstep == palacecoordin) {
						enemypalace.push_back(it->second);
					}
					else {
					...
					auto newstep = it->second->moving(*this, std::make_pair(it->first.first - 1, it->first.second), 2);
					if (newstep == palacecoordin) {
						enemypalace.push_back(it->second);
					}
					else {
	...
	void Playng_field::setCoordin(std::pair<int, int> plc, std::pair<int, int>lr, std::vector<std::pair<int, int>> twr) {

		for (int i = 0; i < counttower; i++)
			quad[twr[i].second][twr[i].first].setType(towertypeempty);
